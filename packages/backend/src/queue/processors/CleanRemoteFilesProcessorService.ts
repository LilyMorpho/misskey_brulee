...
export class CleanRemoteFilesProcessorService {
	...
	private async fixAvatars(): Promise<void> {
		...
		while (true) {
			...
			const results = await Promise.all(
				users.map((user) => this.fixAvatar(user)),
			);
	...
	private async fixAvatar(user: MiUser | null): Promise<boolean> {
		...
		if (user.avatarId) {
			const avatarFile = await this.driveFilesRepository.findOneBy({
				id: user.avatarId,
			});
		...
		if (user.bannerId) {
			const bannerFile = await this.driveFilesRepository.findOneBy({
				id: user.bannerId,
			});
